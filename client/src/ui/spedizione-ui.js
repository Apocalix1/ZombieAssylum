...
function generaPergamenaDaRarita(rarita) {
    const livelloMap = { non_comune: 1, raro: 2, super_raro: 3 };
    const livello = livelloMap[rarita];
    if (livello === undefined) return null;
    const id = `perg-${Date.now()}-${Math.floor(Math.random() * 99999)}`;
    const vuota = Math.random() < 0.45;
    if (vuota) {
        return { id, tipo: 'vuota', maxLivello: livello, spellNome: null };
    }
    const candidati = (window.getSpellDatabaseFlat ? window.getSpellDatabaseFlat() : []).filter(sp => sp.livello === livello);
    if (!candidati.length) return { id, tipo: 'vuota', maxLivello: livello, spellNome: null };
    const spell = candidati[Math.floor(Math.random() * candidati.length)];
    return { id, tipo: 'scritta', livello, spellNome: spell.nome };
}
window.generaPergamenaDaRarita = generaPergamenaDaRarita;

// Estrae eventuali pergamene dal pool di oggetti magici trovati (25% per oggetto non_comune+).
// Modifica direttamente oggMagiciTrovati sottraendo gli oggetti convertiti in pergamene.
function estraiPergameneDaLootMagico(oggMagiciTrovati) {
    const pergameneOttenute = [];
    const mappa = [['nonComuni', 'non_comune'], ['rari', 'raro'], ['superRari', 'super_raro']];
    mappa.forEach(([chiave, rarita]) => {
        let count = oggMagiciTrovati[chiave] || 0;
        for (let i = 0; i < count; i++) {
            if (Math.random() < 0.25) {
                const perg = generaPergamenaDaRarita(rarita);
                if (perg) {
                    pergameneOttenute.push(perg);
                    oggMagiciTrovati[chiave] -= 1;
                }
            }
        }
    });
    return pergameneOttenute;
}
window.estraiPergameneDaLootMagico = estraiPergameneDaLootMagico;
...
function terminaEsplorazione(p) {
    ...
    try {
        ...
        const pergameneTrovate = estraiPergameneDaLootMagico(oggMagiciTrovati);
        ...
        if (pergameneTrovate.length > 0) {
            magazzino.pergamene = magazzino.pergamene || [];
            pergameneTrovate.forEach(p => magazzino.pergamene.push(p));
        }
        ...
        if (pergameneTrovate.length > 0) {
            infoMagica += `• 📜 Pergamene: +${pergameneTrovate.length} (${pergameneTrovate.map(p => p.tipo === 'vuota' ? `vuota Lv${p.maxLivello}` : `"${p.spellNome}"`).join(', ')})\n`;
        }
